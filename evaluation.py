...
def test():
    ...
    for root, _, filenames in os.walk(args.gt_path):
        for i, gt_filename in enumerate(fnmatch.filter(filenames[:len(pred_filenames)], '*.png')):
            if not (pred_filenames[i][-16:-4] == gt_filename[-16:-4]):
                print("Fail")
            dirname = root.replace(args.gt_path, '')
            gt_filenames.append(os.path.join(dirname, gt_filename))
    ...
    if args.dataset == 'kitti':
        for t_id in range(num_test_samples):
            gt_depth_path = os.path.join(args.gt_path, gt_filenames[i])
...
def eval(pred_depths):
    ...
    for i in range(num_samples):
        ...
        # only take the ground truth pixel where we really know the depth 
        valid_mask = np.logical_and(gt_depth > args.min_depth_eval, gt_depth < args.max_depth_eval)

        # IMPORTANT: they should already be cropped in the val_selection_cropped
        # if args.do_kb_crop:
