...
class MiDaS(DepthEstimator):
    def __init__(self, device, model_type, optimize=False, to_metric=True):
        ...
        self.model, self.transform, self.net_w, self.net_h = load_model(
            device, model_path, model_type, optimize, height=None, square=False
        )
        if to_metric:
            self.first_aligning_run = True
        ...
        with torch.no_grad():
            ...
            if "openvino" in self.model_type:
                if first_execution or not use_camera:
                    print(
                        f"    Input resized to {self.net_w}x{self.net_h} before entering the encoder"
                    )
                ...
                if not self.first_aligning_run:
                    ...
                    prediction_aligned[prediction_aligned < self.disparity_cap] = (
                        self.disparity_cap
                    )
    ...
    def compute_scale_and_shift(self):
        ...
        depth_map, mask = depth_read(
            "depth_selection/val_selection_cropped/groundtruth_depth/2011_10_03_drive_0047_sync_groundtruth_depth_0000000791_image_03.png"
        )
        ...
        prediction = self.predict(
            "depth_selection/val_selection_cropped/image/2011_10_03_drive_0047_sync_image_0000000791_image_03.png"
        )
        prediction[prediction < 0] = 0
        ...
        s, t = self.small_alignment(
            d=prediction_flatten, d_star=target_disparity_flatten, points=points
        )
...
class MonoDepth2(DepthEstimator):
    def __init__(self, device, model_name, pred_metric_depth="True"):
        ...
        self.encoder, self.decoder, self.net_w, self.net_h = load_mono2_model(
            model_name, device
        )
...
class DPT(DepthEstimator):
    def __init__(self, device, model_type, optimize=True, to_metric=True):
        ...
        self.model_type = model_type
        self.model, self.transform, self.net_w, self.net_h = load_dpt_model(
            device, model_type, model_path, optimize
        )
    ...
    def predict(self, img_path):
        ...
        with torch.no_grad():
            ...
            if self.optimize is True and self.device == torch.device("cuda"):
                sample = sample.to(memory_format=torch.channels_last)
                sample = sample.half()
...
class ZoeDepth(DepthEstimator):
    def __init__(self, device, model_type):
        if model_type not in zoe_model_types:
            raise TypeError(
                "Model type is not supported by ZoeDepth: use one of [ZoeD_N, ZoeD_K, ZoeD_NK]"
            )
